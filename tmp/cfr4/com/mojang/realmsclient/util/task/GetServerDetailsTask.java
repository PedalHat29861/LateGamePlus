...
extends LongRunningTask {
    private static final Component APPLYING_PACK_TEXT = Component.translatable((String)"multiplayer.applyingPack");
    private static final Logger LOGGER = LogUtils.getLogger();
    private static final Component TITLE = Component.translatable((String)"mco.connect.connecting");
        ...
        try {
            ...
            switch (e.realmsError.errorCode()) {
                ...
                case 6006: {
                    ...
                    GetServerDetailsTask.setScreen(isOwner ? new RealmsBrokenWorldScreen(this.lastScreen, this.server.id, this.server.isMinigameActive()) : new RealmsGenericErrorScreen((Component)Component.translatable((String)"mco.brokenworld.nonowner.title"), (Component)Component.translatable((String)"mco.brokenworld.nonowner.error"), this.lastScreen));
                    return;
                }
            }
            this.error(e);
            ...
            this.error((Component)Component.translatable((String)"mco.errorMessage.connectionFailure"));
            return;
        }
        catch (Exception e) {
            LOGGER.error("Couldn't connect to world", (Throwable)e);
    ...
    private PopupScreen resourcePackDownloadConfirmationScreen(RealmsJoinInformation address, UUID packId, Function<RealmsJoinInformation, Screen> onCompletionScreen) {
        MutableComponent popupMessage = Component.translatable((String)"mco.configure.world.resourcepack.question");
        return RealmsPopups.infoPopupScreen(this.lastScreen, (Component)popupMessage, popupScreen -> {
            ...
            ((CompletableFuture)this.scheduleResourcePackDownload(address, packId).thenRun(() -> GetServerDetailsTask.setScreen((Screen)onCompletionScreen.apply(address)))).exceptionally(e -> {
                ...
                GetServerDetailsTask.setScreen(new RealmsGenericErrorScreen((Component)Component.translatable((String)"mco.download.resourcePack.fail"), this.lastScreen));
                return null;
            });
        });
    }
