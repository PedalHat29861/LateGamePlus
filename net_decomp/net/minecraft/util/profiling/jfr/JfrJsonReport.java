...
public class JfrJsonReport {
    ...
    private JsonElement collectStructureGenSection(List<StructureGenerationSample> structureGenerationSamples) {
        ...
        structureGenerationSamples.stream().collect(Collectors.groupingBy(StructureGenerationSample::structureName)).forEach((name, samples) -> {
            Optional optional = LongRunningSampleStatistics.fromSamples(samples);
            if (optional.isEmpty()) {
                return;
            }
            LongRunningSampleStatistics longRunningSampleStatistics2 = optional.get();
            JsonObject jsonObject2 = new JsonObject();
            jsonArray.add((JsonElement)jsonObject2);
            jsonObject2.addProperty("name", name);
            jsonObject2.addProperty(COUNT, (Number)longRunningSampleStatistics2.count());
            jsonObject2.addProperty(DURATION_NANOS_TOTAL, (Number)longRunningSampleStatistics2.totalDuration().toNanos());
            jsonObject2.addProperty("durationNanosAvg", (Number)(longRunningSampleStatistics2.totalDuration().toNanos() / (long)longRunningSampleStatistics2.count()));
            JsonObject jsonObject3 = Util.make(new JsonObject(), json -> jsonObject2.add("durationNanosPercentiles", (JsonElement)json));
            longRunningSampleStatistics2.quantiles().forEach((quantile, value) -> jsonObject3.addProperty("p" + quantile, (Number)value));
