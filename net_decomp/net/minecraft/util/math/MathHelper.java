...
public class MathHelper {
    ...
    private static final float[] SINE_TABLE = Util.make(new float[65536], sineTable -> {
        for (int i = 0; i < ((float[])sineTable).length; ++i) {
            sineTable[i] = (float)java.lang.Math.sin((double)i / 10430.378350470453);
        }
    });
    ...
    public static int getChebyshevDistance(int x1, int y1, int x2, int y2) {
        return MathHelper.absMax(x2 - x1, y2 - y1);
    }
    ...
    public static int lerpPositive(float delta, int start, int end) {
        int i = end - start;
        return start + MathHelper.floor(delta * (float)(i - 1)) + (delta > 0.0f ? 1 : 0);
    }
    ...
    public static double clampedMap(double value, double oldStart, double oldEnd, double newStart, double newEnd) {
        return MathHelper.clampedLerp(MathHelper.getLerpProgress(value, oldStart, oldEnd), newStart, newEnd);
    }
    ...
    public static IntStream stream(int seed, int lowerBound, int upperBound, int steps) {
        ...
        int i2 = MathHelper.clamp(seed, lowerBound, upperBound);
        return IntStream.iterate(i2, i -> {
            int m = java.lang.Math.abs(i2 - i);
            return i2 - m >= lowerBound || i2 + m <= upperBound;
        }, i -> {
            int o;
            boolean bl2;
            boolean bl = i <= i2;
            int n = java.lang.Math.abs(i2 - i);
            boolean bl3 = bl2 = i2 + n + steps <= upperBound;
            if (!(bl && bl2 || (o = i2 - n - (bl ? steps : 0)) < lowerBound)) {
                return o;
            }
            return i2 + n + steps;
        });
