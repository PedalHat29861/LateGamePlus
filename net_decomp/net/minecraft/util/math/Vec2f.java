...
public class Vec2f {
    ...
    public static final Codec<Vec2f> CODEC = Codec.FLOAT.listOf().comapFlatMap(rawList -> Util.decodeFixedLengthList(rawList, 2).map(list -> new Vec2f(((Float)list.get(0)).floatValue(), ((Float)list.get(1)).floatValue())), vec -> List.of(Float.valueOf(vec.x), Float.valueOf(vec.y)));
