...
implements BiomeAccess.Storage {
    public static final MapCodec<FixedBiomeSource> CODEC = Biome.REGISTRY_CODEC.fieldOf("biome").xmap(FixedBiomeSource::new, biomeSource -> biomeSource.biome).stable();
    ...
    public @Nullable Pair<BlockPos, RegistryEntry<Biome>> locateBiome(BlockPos origin, int radius, int horizontalBlockCheckInterval, int verticalBlockCheckInterval, Predicate<RegistryEntry<Biome>> predicate, MultiNoiseUtil.MultiNoiseSampler noiseSampler, WorldView world) {
        return predicate.test(this.biome) ? Pair.of((Object)origin.withY(MathHelper.clamp(origin.getY(), world.getBottomY() + 1, world.getTopYInclusive() + 1)), this.biome) : null;
    }
