...
public final class BelowZeroRetrogen {
    ...
    private static final Codec<ChunkStatus> STATUS_CODEC = Registries.CHUNK_STATUS.getCodec().comapFlatMap(status -> status == ChunkStatus.EMPTY ? DataResult.error(() -> "target_status cannot be empty") : DataResult.success((Object)status), Function.identity());
    ...
    public void fillColumnsWithAirIfMissingBedrock(ProtoChunk chunk) {
        ...
        for (int k = 0; k < 16; ++k) {
            for (int l = 0; l < 16; ++l) {
                ...
                BlockPos.iterate(k, i, l, k, j, l).forEach(pos -> chunk.setBlockState((BlockPos)pos, Blocks.AIR.getDefaultState()));
            }
        }
    }
