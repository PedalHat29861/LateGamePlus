...
extends ChunkGenerator {
    ...
    public void buildSurface(ChunkRegion region, StructureAccessor structures, NoiseConfig noiseConfig, Chunk chunk) {
        ...
        this.buildSurface(chunk, heightContext, noiseConfig, structures, region.getBiomeAccess(), (Registry<Biome>)region.getRegistryManager().getOrThrow(RegistryKeys.BIOME), Blender.getBlender(region));
    }
    ...
    public CompletableFuture<Chunk> populateNoise(Blender blender, NoiseConfig noiseConfig, StructureAccessor structureAccessor, Chunk chunk) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            ...
            try {
                Chunk chunk2 = this.populateNoise(blender, structureAccessor, noiseConfig, chunk, j, k);
                return chunk2;
            }
            finally {
    ...
    private BlockState getBlockState(ChunkNoiseSampler chunkNoiseSampler, int x, int y, int z, BlockState state) {
        int i;
        int j;
        if (SharedConstants.AQUIFERS && z >= 0 && z % 4 == 0 && y == (j = (i = chunkNoiseSampler.estimateSurfaceHeight(x, z)) + 8)) {
