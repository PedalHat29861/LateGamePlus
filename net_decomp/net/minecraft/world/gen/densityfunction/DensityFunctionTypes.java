...
public final class DensityFunctionTypes {
    ...
    public static MapCodec<? extends DensityFunction> registerAndGetDefault(Registry<MapCodec<? extends DensityFunction>> registry) {
        ...
        DensityFunctionTypes.register(registry, "y_clamped_gradient", YClampedGradient.CODEC_HOLDER);
        return DensityFunctionTypes.register(registry, "find_top_surface", FindTopSurface.CODEC_HOLDER);
    }
    ...
    protected record UnaryOperation(Type type, DensityFunction input, double minValue, double maxValue) implements Unary
    {
        public static UnaryOperation create(Type type, DensityFunction input) {
            ...
            double f = UnaryOperation.apply(type, d);
            double g = UnaryOperation.apply(type, e);
            if (type == Type.INVERT) {
                if (d < 0.0 && e > 0.0) {
                    return new UnaryOperation(type, input, Double.NEGATIVE_INFINITY, Double.POSITIVE_INFINITY);
                }
                return new UnaryOperation(type, input, g, f);
            }
            if (type == Type.ABS || type == Type.SQUARE) {
                return new UnaryOperation(type, input, Math.max(0.0, d), Math.max(f, g));
            }
            return new UnaryOperation(type, input, f, g);
        }
        ...
        implements StringIdentifiable {
            ...
            public static final /* enum */ Type INVERT = new Type("invert");
    ...
    {
        private static final MapCodec<FindTopSurface> FIND_TOP_SURFACE_CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group((App)DensityFunction.FUNCTION_CODEC.fieldOf("density").forGetter(FindTopSurface::density), (App)DensityFunction.FUNCTION_CODEC.fieldOf("upper_bound").forGetter(FindTopSurface::upperBound), (App)Codec.intRange((int)(DimensionType.MIN_HEIGHT * 2), (int)(DimensionType.MAX_COLUMN_HEIGHT * 2)).fieldOf("lower_bound").forGetter(FindTopSurface::lowerBound), (App)Codecs.POSITIVE_INT.fieldOf("cell_height").forGetter(FindTopSurface::cellHeight)).apply((Applicative)instance, FindTopSurface::new));
        public static final CodecHolder<FindTopSurface> CODEC_HOLDER = DensityFunctionTypes.holderOf(FIND_TOP_SURFACE_CODEC);

        @Override
        public double sample(DensityFunction.NoisePos pos) {
            int i = MathHelper.floor(this.upperBound.sample(pos) / (double)this.cellHeight) * this.cellHeight;
            if (i <= this.lowerBound) {
                return this.lowerBound;
            }
            for (int j = i; j >= this.lowerBound; j -= this.cellHeight) {
                DensityFunction.UnblendedNoisePos unblendedNoisePos = new DensityFunction.UnblendedNoisePos(pos.blockX(), j, pos.blockZ());
                if (!(this.density.sample(unblendedNoisePos) > 0.0)) continue;
                return j;
            }
            return this.lowerBound;
        }
        ...
        @Override
        public DensityFunction apply(DensityFunction.DensityFunctionVisitor visitor) {
            return visitor.apply(new FindTopSurface(this.density.apply(visitor), this.upperBound.apply(visitor), this.lowerBound, this.cellHeight));
        }
