...
public class SaveVersionInfo {
    ...
    public static SaveVersionInfo fromDynamic(Dynamic<?> dynamic) {
        ...
        if (optionalDynamic.result().isPresent()) {
            return new SaveVersionInfo(i, l, optionalDynamic.get("Name").asString(SharedConstants.getGameVersion().name()), optionalDynamic.get("Id").asInt(SharedConstants.getGameVersion().dataVersion().id()), optionalDynamic.get("Series").asString("main"), optionalDynamic.get("Snapshot").asBoolean(!SharedConstants.getGameVersion().stable()));
        }
        return new SaveVersionInfo(i, l, "", 0, "main", false);
    }
